...
module.exports.run = async (client, message, args) => {
    ...
    let salvoembed = new Discord.MessageEmbed().setColor("RANDOM").setFooter(`${ayarlarlar.footer}`).setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true}))
    ...
    if(!isim) return message.channel.send(salvoembed.setDescription(`Lütfen Bir İsim Yazınız \`Örnek: ${ayarlar.prefix}kayıtsız @üye Ses Teyit Alınacak\` `)).then(m => m.delete({timeout: 7000}));
    ...
    victim.roles.cache.has(ayarlar.boosterRol) ? victim.roles.set([ayarlar.boosterRol, ayarlar.kayıtsızRol]) : victim.roles.set([ayarlar.kayıtsızRol]);
    if (victim.voice.channel) victim.voice.kick();
    client.channels.cache.get(ayarlar.kayıtLogKanalı).send(salvoembed.setDescription(`**__Bir Kullanıcı Kayıtsıza Atıldı__**
